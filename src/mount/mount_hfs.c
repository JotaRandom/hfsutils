/*
 * mount_hfs.c - HFS filesystem mount implementation
 * Copyright (C) 2025
 */
...
static int verify_hfs_filesystem(const char *device) {
    int fd;
    unsigned char sig[2];
    ...
    /* Read signature at offset 1024 (MDB) */
...
int mount_hfs_volume(const char *device, const char *mountpoint, 
    ...
    /* Verify filesystem */
    if (verify_hfs_filesystem(device) < 0) {
    ...
    /* Attempt mount */
    if (mount(device, mountpoint, "hfs", flags, NULL) < 0) {
        ...
        if (errno == EACCES || errno == EPERM) {
            ...
            fprintf(stderr, "mount.hfs: filesystem type 'hfs' not supported by kernel\n");
            fprintf(stderr, "mount.hfs: try loading the hfs kernel module: modprobe hfs\n");
            return MOUNT_FAILURE;
        } else {
