...
int main(int argc, char *argv[])
{
  ...
  /* Check if called as 'hfsutil' with a subcommand */
  if (strcmp(bargv0, "hfsutil") == 0 || strncmp(bargv0, "hfsutil", len) == 0)
    {
      if (argc < 2)
        {
          fprintf(stderr, "HFS Utilities - %s\n", hfsutils_version);
          fprintf(stderr, "\nUsage: %s <command> [options]\n", argv0);
          fprintf(stderr, "\nAvailable commands:\n");
          for (i = 0; list[i].name; ++i)
            {
              if (strcmp(list[i].name, "hdir") != 0)  /* Skip hdir (alias for hls) */
                fprintf(stderr, "  %-10s", list[i].name);
              if ((i % 5) == 4)
                fprintf(stderr, "\n");
            }
          fprintf(stderr, "\n\nFor help on a specific command: %s <command> -h\n", argv0);
          fprintf(stderr, "For version info: %s --version\n", argv0);
          fprintf(stderr, "For license info: %s --license\n", argv0);
          return 1;
        }

      /* Use the first argument as the command */
      bargv0 = argv[1];
      argv[1] = argv[0];  /* Shift argv[0] to argv[1] for compatibility */
      argv++;
      argc--;
    }
  ...
  fprintf(stderr, "Run '%s' without arguments for a list of commands.\n", argv0);
  return 1;
}
